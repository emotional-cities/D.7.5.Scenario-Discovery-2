...
# #Save the results
# with open('Y_train_1_PT_' + mode + '.pickle', 'wb') as handle:
#     pickle.dump(y1, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open('Y_train_2_PT_' + mode + '.pickle', 'wb') as handle:
#     pickle.dump(y2, handle, protocol=pickle.HIGHEST_PROTOCOL)

import pandas as pd
import pickle
with open('Y_train_2_PT_new'+ mode + '.pickle', 'rb') as handle:
    y2 = pickle.load(handle)
with open('Y_train_4_PT_new'+ mode + '.pickle', 'rb') as handle:
    y4 = pickle.load(handle)
...
first_column_percentile = np.percentile(y_new_labeled, 20)
...
for j in range(n_iter):
   ...
   #perform PRIM in the posterior LHS
    ...
    outcome1 = np.array([outcome[1]])
    outcome2 = np.array([outcome[3]])
...
with open('Prediction_test_PRIM_PT_1_dim_UNC_sampling_2' + mode + '.pickle', 'wb') as handle:
    pickle.dump(predictions, handle, protocol=pickle.HIGHEST_PROTOCOL)
#Save the list with the results
with open('X_final_PRIM_PT_1_dim_UNC_sampling_2' + mode + '.pickle', 'wb') as handle:
    pickle.dump(X_new_labeled, handle, protocol=pickle.HIGHEST_PROTOCOL)
#Save the list with the results
with open('Y_final_PRIM_PT_1_dim_UNC_sampling_2'+ mode + '.pickle', 'wb') as handle:
    pickle.dump(y_new_labeled, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open('Y_final_total_women_trips_dim_2_sampling_2'+ mode + '.pickle', 'wb') as handle:
